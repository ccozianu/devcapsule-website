...
test("Markdown routes, anchors, queries and encoded paths preserve navigation", () => {
  assert.equal(
    ...
    "/docs/guides/windows-wsl2/#before-you-start",
  );
  assert.equal(
    rewriteLink(
      "../../README.md?from=docs#why-devcapsule",
      "docs/guides/first-session.md",
      files,
      "/unused",
      sha,
    ...
    "/?from=docs#why-devcapsule",
  );
  assert.equal(route("engineering-docs/blog/README.md"), "/blog/");
  const permalink =
    "https://github.com/ccozianu/devcapsule/blob/" + sha + "/AGENTS.md";
  assert.equal(
    rewriteLink(permalink, "README.md", files, "/unused", sha, new Set()),
    permalink,
  );
});
test("non-published content links use source revision; missing files fail", () => {
  ...
  try {
    ...
    assert.throws(
      () => rewriteLink("missing.md", "README.md", files, dir, sha, new Set()),
      /Broken source link/,
    );
    assert.throws(
      () =>
        rewriteLink("../../private", "README.md", files, dir, sha, new Set()),
      /escapes/,
    );
  } finally {
