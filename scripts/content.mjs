...
export function rewriteLink(href, file, files, dir, revision, assets) {
  ...
  if (target.startsWith("../"))
    throw new Error(`Link escapes content root: ${file}: ${href}`);
  ...
  if (/\.(?:png|jpg|jpeg|gif|svg|webp|avif)$/i.test(target)) {
    if (!fs.existsSync(path.join(dir, target)))
      throw new Error(`Missing image: ${target}`);
    assets.add(target);
    return siteUrl("/content-assets/" + encodePath(target)) + suffix;
  }
  if (!fs.existsSync(path.join(dir, target)))
    throw new Error(`Broken source link in ${file}: ${href}`);
...
export function render(source, file, files, dir, revision, assets) {
  ...
  $("a[href], img[src]").each((_, element) => {
    ...
    el.attr(
      attr,
      rewriteLink(el.attr(attr), file, files, dir, revision, assets),
    );
  });
  $("table").each((i, el) =>
    $(el).wrap(
...
export function buildContent() {
  ...
  const filenames = [
    ...
    ...walk(path.join(dir, "engineering-docs/blog"))
      .filter((p) => p.endsWith(".md"))
      .map((p) => path.relative(dir, p)),
  ];
  ...
  const pages = filenames
    .filter((file) => file !== "engineering-docs/blog/README.md")
    .map((file) => {
      ...
      const kind =
        file === "README.md"
          ? "home"
          : file.startsWith("engineering-docs/blog/")
            ? "post"
            : "doc";
      ...
      return {
        ...
        excerpt:
          rendered.description.length > 210
            ? rendered.description.slice(0, 207).replace(/\s+\S*$/, "") + "…"
            : rendered.description,
        historical:
          file.includes("/product/") || file.includes("docker4pycharm"),
  ...
  const navFiles = [
    "docs/README.md",
    "docs/guides/first-session.md",
    "docs/guides/your-project.md",
    "docs/guides/windows-wsl2.md",
  ];
  const labels = [
    "Overview",
    "Your first session",
  ...
  const navigation = navFiles.map((file, i) => ({
    ...pages.find((p) => p.file === file),
    label: labels[i],
  }));
  navigation.push(
    ...pages
      .filter(
        (p) => p.kind === "doc" && !p.historical && !navFiles.includes(p.file),
      )
